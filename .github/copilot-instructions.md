...
- **SPDX ヘッダー必須**: AGPL-3.0-only 管轄かつ SPDX CI 対象ディレクトリに新規 `.ts` / `.js` / `.cjs` / `.mjs` / `.scss` / `.vue` / `.html` ファイルを追加する場合は冒頭に必ず付ける。対象判定は `scripts/check-spdx.mjs` を参照。
...
1. ESLint 対象の変更ファイルへ package root から `eslint --quiet` を最後に 1 回実行し、実装変更には最も近い test を選んで実行する。package / repo 全体 lint と広域 test は任意
...
5. ユーザーが明示しない限り `CHANGELOG.md` を編集しない。ユーザー影響がある変更では引き継ぎに候補を 1 行だけ示す
