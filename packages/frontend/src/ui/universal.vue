...
watch(navFooter, () => {
	if (navFooter.value) {
		...
		document.body.style.setProperty('--MI_UI-minBottomSpacing', 'var(--MI_UI-minBottomSpacingMobile)');
	} else {
		navFooterHeight.value = 0;
		document.body.style.setProperty('--stickyBottom', '0px');
		document.body.style.setProperty('--MI_UI-minBottomSpacing', '0px');
	}
}, {
	immediate: true,
});
...
.widgetsDrawer {
	...
	width: 310px;
	height: 100dvh;
	padding: var(--MI_UI-margin) var(--MI_UI-margin) calc(var(--MI_UI-margin) + env(safe-area-inset-bottom, 0px)) !important;
	box-sizing: border-box;
	overflow: auto;
	overscroll-behavior: contain;
	background: var(--MI_THEME-bg);
...
.nav {
	...
	width: 100%;
	box-sizing: border-box;
	-webkit-backdrop-filter: var(--MI_UI-blur, blur(24px));
	backdrop-filter: var(--MI_UI-blur, blur(24px));
...
.spacer {
	height: calc(var(--MI_UI-minBottomSpacing));
}
</style>
