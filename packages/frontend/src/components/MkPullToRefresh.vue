...
function moveStart(event: PointerEvent) {
	...
	rootEl.value.addEventListener('pointermove', moving, { passive: false }); // passive: falseにしないとpreventDefaultが使えない
...
function moveEnd(event: PointerEvent) {
	rootEl.value.removeEventListener('pointermove', moving);
