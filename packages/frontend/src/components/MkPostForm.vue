  ...
  >
    <header :class="$style.header">
      ...
      <div :class="$style.headerRight">
        <span :class="[$style.textCount, { [$style.textOver]: textLength > maxTextLength }]">{{
        ...
        <button
          ...
          class="_button"
          :class="[$style.previewButton, { [$style.previewButtonActive]: showPreview }]"
        ...
        >
          <div :class="$style.submitInner">
            ...
            <i
              ...
              :class="posted ? 'ti ti-check' : reply ? 'ti ti-arrow-back-up' : renote ? 'ti ti-quote' : 'ti ti-send'"
            ></i>
          </div>
        </button>
      </div>
    ...
    <div :class="[$style.form]">
      ...
      <div v-if="visibility === 'specified'" :class="$style.toSpecified">
        ...
        <div :class="$style.visibleUsers">
          <span v-for="u in visibleUsers" :key="u.id" :class="$style.visibleUser">
            <MkAcct :user="u" />
            <button class="_button" style="padding: 4px 8px" @click="removeVisibleUser(u)">
              <i class="ti ti-x"></i>
            </button>
          </span>
          <button class="_buttonPrimary" style="padding: 4px; border-radius: 8px" @click="addVisibleUser">
      ...
      <footer :class="$style.footer">
        ...
        <button v-tooltip="i18n.ts.mention" class="_button" :class="$style.footerButton" @click="insertMention">
          <i class="ti ti-at"></i>
        </button>
        <button
...
const submitText = $computed((): string => {
  return props.renote ? i18n.ts.quote : props.reply ? i18n.ts.reply : i18n.ts.note;
});
...
if (
  props.reply &&
  (props.reply.user.username !== $i.username || (props.reply.user.host != null && props.reply.user.host !== host))
) {
...
async function onPaste(ev: ClipboardEvent) {
  for (const { item, i } of Array.from(ev.clipboardData.items).map((item, i) => ({ item, i }))) {
    if (item.kind === 'file') {
      ...
      const formatted = `${formatTimeString(new Date(file.lastModified), defaultStore.state.pastedFileName).replace(
        /{{number}}/g,
        `${i + 1}`,
      )}${ext}`;
