...
const fetchFederationChart = async (): Promise<typeof chartData> => {
	const raw = await apiGet('charts/federation', { limit: props.limit, span: props.span });
	return {
		series: [{
			name: 'Received',
			type: 'area',
...
const fetchApRequestChart = async (): Promise<typeof chartData> => {
	const raw = await apiGet('charts/ap-request', { limit: props.limit, span: props.span });
	return {
		series: [{
			name: 'In',
			type: 'area',
...
const fetchNotesChart = async (type: string): Promise<typeof chartData> => {
	const raw = await apiGet('charts/notes', { limit: props.limit, span: props.span });
	return {
		series: [{
			name: 'All',
			type: 'line',
...
const fetchUsersChart = async (total: boolean): Promise<typeof chartData> => {
	const raw = await apiGet('charts/users', { limit: props.limit, span: props.span });
	return {
		series: [{
			name: 'Combined',
			type: 'line',
...
const fetchActiveUsersChart = async (): Promise<typeof chartData> => {
	const raw = await apiGet('charts/active-users', { limit: props.limit, span: props.span });
	return {
		series: [{
			name: 'Read & Write',
			type: 'area',
...
const fetchDriveChart = async (): Promise<typeof chartData> => {
	const raw = await apiGet('charts/drive', { limit: props.limit, span: props.span });
	return {
		bytes: true,
		series: [{
			name: 'All',
...
const fetchInstanceRequestsChart = async (): Promise<typeof chartData> => {
	const raw = await apiGet('charts/instance', { host: props.args.host, limit: props.limit, span: props.span });
	return {
		series: [{
			name: 'In',
			type: 'area',
...
const fetchPerUserNotesChart = async (): Promise<typeof chartData> => {
	const raw = await apiGet('charts/user/notes', { userId: props.args.user.id, limit: props.limit, span: props.span });
	return {
...
const fetchPerUserPvChart = async (): Promise<typeof chartData> => {
	const raw = await apiGet('charts/user/pv', { userId: props.args.user.id, limit: props.limit, span: props.span });
	return {
		series: [{
			name: 'Unique PV (user)',
			type: 'area',
...
const fetchPerUserFollowingChart = async (): Promise<typeof chartData> => {
	const raw = await apiGet('charts/user/following', { userId: props.args.user.id, limit: props.limit, span: props.span });
	return {
		series: [{
			name: 'Local',
			type: 'area',
...
const fetchPerUserDriveChart = async (): Promise<typeof chartData> => {
	const raw = await apiGet('charts/user/drive', { userId: props.args.user.id, limit: props.limit, span: props.span });
	return {
		series: [{
			name: 'Inc',
			type: 'area',
