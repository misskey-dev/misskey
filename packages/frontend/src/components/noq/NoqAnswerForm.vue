...
// 回答から引用部分を除いた純粋な回答テキスト
const pureAnswerText = computed(() => {
	const text = answerText.value;
	const answerMarker = 'A. ';
	const idx = text.indexOf(answerMarker);
	if (idx !== -1) {
		return text.substring(idx + answerMarker.length).trim();
	}
	return text.trim();
});

// 質問テキストを引用として含める
const quotedQuestion = computed(() => {
	if (props.question.sender) {
		return `Q. ${props.question.text}\n\nA. `;
	}
	return `Q. ${props.question.text}（匿名）\n\nA. `;
});

onMounted(() => {
	// 質問テキストを引用として初期表示
	answerText.value = quotedQuestion.value;
	// 絵文字ピッカーを初期化
	emojiPicker.init();
});
