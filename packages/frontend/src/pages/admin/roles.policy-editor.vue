...
<div class="_gaps_s">
	...
	<XFolder v-if="matchQuery([i18n.ts._role._options.spReactionsMonthlyLimit, 'spReactionsMonthlyLimit'])" v-model:policyMeta="policyMetaModel.spReactionsMonthlyLimit" :isBaseRole="isBaseRole" :readonly="readonly">
		<template #label>{{ i18n.ts._role._options.spReactionsMonthlyLimit }}</template>
		<template #valueText>{{ valuesModel.spReactionsMonthlyLimit }}</template>
		<template #default="{ disabled }">
			<MkInput v-model="valuesModel.spReactionsMonthlyLimit" type="number" :disabled="disabled" :min="0">
			</MkInput>
		</template>
	</XFolder>
</div>
