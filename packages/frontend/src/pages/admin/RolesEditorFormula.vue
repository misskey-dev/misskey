	...
	<div class="_gaps">
		<div :class="$style.header">
			...
			<button
				...
				:draggable="true"
				@dragstart.stop="dragStartCallback"
			>
		...
		<div v-if="v.type === 'and' || v.type === 'or'" class="_gaps">
			...
			<MkButton rounded style="margin: 0 auto" @click="addChildValue"
		...
		<MkInput
			v-else-if="
				v.type === 'createdLessThan' ||
				v.type === 'createdMoreThan' ||
		...
		<MkInput
			v-else-if="
				v.type === 'followersLessThanOrEq' ||
				v.type === 'followersMoreThanOrEq' ||
				v.type === 'followingLessThanOrEq' ||
				v.type === 'followingMoreThanOrEq' ||
				v.type === 'notesLessThanOrEq' ||
				v.type === 'notesMoreThanOrEq'
			"
			v-model="v.value"
			type="number"
		>
		...
		<MkSelect
			v-else-if="v.type === 'roleAssignedTo'"
			v-model="v.roleId"
			:items="assignedToDef"
		>
		</MkSelect>
...
import type {
	GetMkSelectValueTypesFromDef,
	MkSelectItem,
} from "@/components/MkSelect.vue";
...
const typeDef = [
	...
	{ label: i18n.ts._role._condition.isInYamiMode, value: "isInYamiMode" },
...
const typeModelForMkSelect = computed<
	GetMkSelectValueTypesFromDef<typeof typeDef>
>({
	get: () => v.value.type,
	set: (t) => {
		...
		switch (t) {
			...
			case "createdLessThan":
				newValue = { type: "createdLessThan", sec: 86400 };
				break;
			case "createdMoreThan":
				newValue = { type: "createdMoreThan", sec: 86400 };
				break;
...
const assignedToDef = computed(
	() =>
		roles
			...
			.map((r) => ({ label: r.name, value: r.id })) satisfies MkSelectItem[],
);
