		...
		<div class="_gaps">
			<MkFolder>
				...
				<div class="_gaps_s">
					...
					<MkFolder v-if="matchQuery([i18n.ts._role._options.canViewCharts, 'canViewCharts'])">
						<template #label>{{ i18n.ts._role._options.canViewCharts }}</template>
						<template #suffix>{{ policies.canViewCharts ? i18n.ts.yes : i18n.ts.no }}</template>
						<MkSwitch v-model="policies.canViewCharts">
							<template #label>{{ i18n.ts.enable }}</template>
						</MkSwitch>
					</MkFolder>
			...
			<div class="_gaps_s">
				<MkFoldableSection>
					...
					<div class="_gaps_s">
						<MkRolePreview v-for="role in roles.filter(x => x.target === 'manual' && !x.isCommunity)" :key="role.id" :role="role" :forModeration="true"/>
					</div>
				</MkFoldableSection>
				<MkFoldableSection>
					<template #header>{{ i18n.ts._role.conditionalRoles }}</template>
					<div class="_gaps_s">
						<MkRolePreview v-for="role in roles.filter(x => x.target === 'conditional' && !x.isCommunity)" :key="role.id" :role="role" :forModeration="true"/>
					</div>
				</MkFoldableSection>
				<MkFoldableSection>
					<template #header>{{ i18n.ts.communityRole }}</template>
					<div class="_gaps_s">
						<MkRolePreview v-for="role in roles.filter(x => x.isCommunity)" :key="role.id" :role="role" :forModeration="true"/>
					</div>
				</MkFoldableSection>
			</div>
		</div>
