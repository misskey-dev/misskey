			...
			<FormSuspense :p="init">
				<div class="_gaps_m">
					...
					<MkInput v-model="app192IconUrl" type="url">
						...
						<template #caption>
							<div>{{ i18n.tsx._serverSettings.appIconDescription(instance.name ?? host) }}</div>
							<div>({{ i18n.ts._serverSettings.appIconUsageExample }})</div>
							<div>{{ i18n.ts._serverSettings.appIconStyleRecommendation }}</div>
							<div><strong>{{ i18n.tsx._serverSettings.appIconResolutionMustBe('192x192px') }}</strong></div>
						</template>
					</MkInput>
					...
					<MkInput v-model="app512IconUrl" type="url">
						...
						<template #caption>
							...
							<div>({{ i18n.ts._serverSettings.appIconUsageExample }})</div>
							<div>{{ i18n.ts._serverSettings.appIconStyleRecommendation }}</div>
							<div><strong>{{ i18n.tsx._serverSettings.appIconResolutionMustBe('512x512px') }}</strong></div>
						</template>
					</MkInput>
