...
function createFetcher() {
	return () => Promise.all([api('users/show', {
		userId: props.userId,
	}), api('admin/show-user', {
		userId: props.userId,
	}), iAmAdmin ? api('admin/get-user-ips', {
		userId: props.userId,
		...
		watch(moderationNote, async () => {
			await api('admin/update-user-note', { userId: user.value.id, text: moderationNote.value });
			await refreshUser();
		});
	});
}
...
async function resetPassword() {
	...
	if (confirm.canceled) {
		...
		const { password } = await api('admin/reset-password', {
...
async function toggleSuspend(v) {
	...
	if (confirm.canceled) {
		...
		await api(v ? 'admin/suspend-user' : 'admin/unsuspend-user', { userId: user.value.id });
		await refreshUser();
	}
}
...
async function unsetUserAvatar() {
	...
	const process = async () => {
		await api('admin/unset-user-avatar', { userId: user.value.id });
		os.success();
	};
...
async function unsetUserBanner() {
	...
	if (confirm.canceled) return;
	const process = async () => {
		await api('admin/unset-user-banner', { userId: user.value.id });
		os.success();
	};
	await process().catch(err => {
		os.alert({
...
async function deleteAllFiles() {
	...
	if (confirm.canceled) return;
	const process = async () => {
		await api('admin/delete-all-files-of-a-user', { userId: user.value.id });
		os.success();
	};
	await process().catch(err => {
		os.alert({
