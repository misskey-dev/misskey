	...
	<div :class="$style.root" class="_gaps">
		...
		<div style="overflow: clip; line-height: 28px;">
			<div v-if="!iAmPlayer && !game.isEnded && turnUser" class="turn">
				<Mfm :key="'turn:' + turnUser.id" :text="i18n.tsx._reversi.turnOf(turnUser.name ?? turnUser.username)" :plain="true" :customEmojis="turnUser.emojis"/>
				<MkEllipsis/>
			</div>
			<div v-if="(logPos !== logs.length) && turnUser" class="turn">
				<Mfm :key="'past-turn-of:' + turnUser.id" :text="i18n.tsx._reversi.pastTurnOf(turnUser.name ?? turnUser.username)" :plain="true" :customEmojis="turnUser.emojis"/>
			</div>
			...
			<div v-if="game.isEnded && logPos == logs.length" class="result">
				<template v-if="game.winner">
					<Mfm :key="'won'" :text="i18n.tsx._reversi.won(game.winner.name ?? game.winner.username)" :plain="true" :customEmojis="game.winner.emojis"/>
		...
		<div class="status"><b>{{ i18n.tsx._reversi.turnCount(logPos) }}</b> {{ i18n.ts._reversi.black }}:{{ engine.blackCount }} {{ i18n.ts._reversi.white }}:{{ engine.whiteCount }} {{ i18n.ts._reversi.total }}:{{ engine.blackCount + engine.whiteCount }}</div>
