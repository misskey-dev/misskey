...
async function init() {
	...
	try {
		...
		if (replyId) {
			reply.value = await api('notes/show', {
				noteId: replyId,
			});
		} else if (replyUri) {
			const obj = await api('ap/show', {
				uri: replyUri,
			});
			if (obj.type === 'Note') {
		...
		if (renoteId) {
			renote.value = await api('notes/show', {
				noteId: renoteId,
			});
		} else if (renoteUri) {
			const obj = await api('ap/show', {
		...
		if (fileIds) {
			await Promise.all(
				fileIds.split(',')
					.map(fileId => api('drive/files/show', { fileId })
						.then(file => {
