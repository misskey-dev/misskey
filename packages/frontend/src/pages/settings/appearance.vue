		...
		<FormSection first>
			<div class="_gaps_m">
				<div class="_gaps_s">
					...
					<SearchMarker :keywords="['font', 'system', 'native', 'custom']">
						<FormSection>
							<template #label><i class="ti ti-typography"></i> <SearchLabel>{{ i18n.ts.fontSettings }}</SearchLabel></template>
							<div class="_gaps_s">
								<!-- システムフォント使用オプション -->
								<MkSwitch v-model="useSystemFont" @update:modelValue="onSystemFontChange">
		...
		<SearchMarker :keywords="['note', 'display']">
			<FormSection>
				...
				<div class="_gaps_m">
					...
					<SearchMarker :keywords="['reaction', 'hide', 'count', 'user', 'mute', 'check']">
						<FormSection>
							<template #label><i class="ti ti-mood-happy"></i> <SearchLabel>{{ i18n.ts.reactionSettings }}</SearchLabel></template>
							<div class="_gaps_s">
								<!-- リアクションユーザー非表示（独自機能） -->
...
// getCustomFontListの代わりにコンポーネント内で実装
...
function onSystemFontChange() {
	if (useSystemFont.value) {
		// システムフォントを使用する場合はカスタムフォント設定をクリア
