...
api('endpoints').then(endpointResponse => {
...
function send() {
	...
	api(endpoint.value as keyof Endpoints, requestBody, requestBody.i || (withCredential.value ? undefined : null)).then(resp => {
		sending.value = false;
...
function onEndpointChange() {
	api('endpoint', { endpoint: endpoint.value }, withCredential.value ? undefined : null).then(resp => {
		const endpointBody = {};
