...
import ShowUsers from '@/server/api/endpoints/admin/show-users.js';
...
describe('suspension consistency', () => {
	...
	async function user(host: string | null = `${secureRndstr(8)}.example.com`) {
		...
		userIds.push(id);
		await users.insert({ id, username: id, usernameLower: id.toLowerCase(), host: host?.toLowerCase() ?? null });
	...
	test('statistics rank active relationships, not stale instance counters', async () => {
		const prefix = secureRndstr(8).toLowerCase();
	...
	test('remote and local suspension remain independent', async () => {
		const actor = await user();
		const target = await user();
		await follow(actor, target);
		await suspension.suspendFromRemote({ id: actor.id, host: actor.host! });
		await suspension.suspend(actor, actor);
		await suspension.unsuspendFromRemote({ id: actor.id, host: actor.host! });
		expect((await followings.findOneByOrFail({ followerId: actor.id })).isFollowerSuspended).toBe(true);
		await suspension.unsuspend(actor, actor);
		expect((await followings.findOneByOrFail({ followerId: actor.id })).isFollowerSuspended).toBe(false);
	});

	test('admin suspended list includes remote suspension', async () => {
		const actor = await user();
		await suspension.suspendFromRemote({ id: actor.id, host: actor.host! });
		const packer = { packMany: async (values: unknown[]) => values } as unknown as UserEntityService;
		const endpoint = new ShowUsers(users, packer, {} as RoleService);
		const moderator = await user(null) as MiLocalUser;
		const params = { state: 'suspended', origin: 'remote', hostname: actor.host };
		const result = await endpoint.exec(params, moderator, null);
		expect(result.map((x: { id: string }) => x.id)).toContain(actor.id);
		expect(await endpoint.exec({ ...params, state: 'available' }, moderator, null)).toEqual([]);
	});
});
