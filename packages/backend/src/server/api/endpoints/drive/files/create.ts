...
export default define(meta, paramDef, async (ps, user, _, file, cleanup, ip, headers) => {
	...
	try {
		// Create file
		const driveFile = await addFile({ user, path: file.path, name, comment: ps.comment, folderId: ps.folderId, force: ps.force, sensitive: ps.isSensitive, requestIp: ip, requestHeaders: headers });
