...
@Module({
	...
	providers: [
		...
		{ provide: 'queue:deliver', useFactory: (config: Config) => initializeQueue('deliver', config.deliverJobPerSec ?? 128), inject: [DI.config] },
		{ provide: 'queue:inbox', useFactory: (config: Config) => initializeQueue('inbox', config.inboxJobPerSec ?? 16), inject: [DI.config] },
