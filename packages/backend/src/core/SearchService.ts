...
export class SearchService {
	...
	public async indexNote(note: MiNote): Promise<void> {
		...
		var attachedFileType = '';
		if ( note.attachedFileTypes ){
			if ( note.attachedFileTypes.length > 0 ){
				const firstF =  note.attachedFileTypes[0];
				if (firstF){
					if( firstF.indexOf('/') > 0 ){
						attachedFileType = firstF.substr(0, firstF.indexOf('/'));
					}
				}
			}
		}
	...
	private async searchNoteByMeiliSearch(
		...
		console.log('----search meilisearch-----');
		console.log(opts);
		if (opts.attachedFileType) {
			console.log('----search meilisearch attachedFileType-----');
			console.log(opts.attachedFileType);
			filter.qs.push({ op: '=', k: 'attachedFileType', v: opts.attachedFileType });
		}
