	...
	protected async tickMinor(): Promise<Partial<KVs<typeof schema>>> {
		...
		const [sub, pub, pubsub, subActive, pubActive] = await Promise.all([
			...
			this.followingsRepository.createQueryBuilder('following')
				...
				.andWhere(meta.blockedHosts.length === 0 ? '1=1' : 'following.followerHost NOT LIKE ANY({:...blocked})', { blocked: meta.blockedHosts.map(x => `%${x}`) })
			...
			this.followingsRepository.createQueryBuilder('following')
				.select('COUNT(DISTINCT following.followeeHost)')
				.where('following.followeeHost IS NOT NULL')
				.andWhere(meta.blockedHosts.length === 0 ? '1=1' : 'following.followeeHost NOT LIKE ANY({:...blocked})', { blocked: meta.blockedHosts.map(x => `%${x}`) })
				.andWhere(`following.followeeHost NOT IN (${ suspendedInstancesQuery.getQuery() })`)
			...
			this.instancesRepository.createQueryBuilder('instance')
				...
				.andWhere(meta.blockedHosts.length === 0 ? '1=1' : 'instance.host NOT LIKE ANY({:...blocked})', { blocked: meta.blockedHosts.map(x => `%${x}`) })
