	...
	protected async tickMinor(): Promise<Partial<KVs<typeof schema>>> {
		...
		const [sub, pub, pubsub, subActive, pubActive] = await Promise.all([
			...
			this.followingsRepository.createQueryBuilder('following')
				...
				.andWhere(meta.blockedHosts.length === 0 ? '1=1' : 'following.followerHost NOT LIKE ANY({:...blocked})', { blocked: meta.blockedHosts.flatMap(x => [x, `%.${x}`]) })
			...
			this.instancesRepository.createQueryBuilder('instance')
				...
				.andWhere(meta.blockedHosts.length === 0 ? '1=1' : 'instance.host NOT LIKE ANY({:...blocked})', { blocked: meta.blockedHosts.flatMap(x => [x, `%.${x}`]) })
