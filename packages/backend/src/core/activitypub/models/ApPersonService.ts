		...
		try {
			...
			await this.db.transaction(async transactionalEntityManager => {
				...
				await transactionalEntityManager.save(new MiUserProfile({
					userId: user.id,
					description: person._misskey_summary ? person._misskey_summary : person.summary ? this.apMfmService.htmlToMfm(truncate(person.summary, summaryLength), person.tag) : null,
					url,
					fields,
