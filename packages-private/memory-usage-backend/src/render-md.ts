...
if (baseFileArg == null || headFileArg == null || outputFileArg == null) {
	console.error('Usage: render-md <baseReport.json> <headReport.json> <output.md>');
	process.exit(1);
}
...
const baseFile = resolve(baseFileArg);
const headFile = resolve(headFileArg);
...
try {
	...
	const baseHeapSnapshotArtifactUrl = readRequiredEnv('MK_MEMORY_HEAP_SNAPSHOT_ARTIFACT_URL_BASE');
	const headHeapSnapshotArtifactUrl = readRequiredEnv('MK_MEMORY_HEAP_SNAPSHOT_ARTIFACT_URL_HEAD');
