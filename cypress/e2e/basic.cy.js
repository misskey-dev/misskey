...
describe('After user signed in', () => {
  ...
  it('successfully loads', () => {
		...
		cy.get('[data-cy-user-setup-continue]', { timeout: 8000 }).should('be.visible');
  });

	it('account setup wizard', () => {
		// 表示に時間がかかるのでデフォルト秒数だとタイムアウトする
		cy.get('[data-cy-user-setup-continue]', { timeout: 8000 }).click();
...
describe('After user setup', () => {
	beforeEach(() => {
		...
		// アカウント初期設定ウィザード
		// 表示に時間がかかるのでデフォルト秒数だとタイムアウトする
		cy.get('[data-cy-user-setup] [data-cy-modal-window-close]', { timeout: 8000 }).click();
		cy.get('[data-cy-modal-dialog-ok]').click();
	});
