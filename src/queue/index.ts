...
deliverQueue
	...
	.on('error', err => deliverLogger.error(`error ${err}`, { e: renderError(err) }))
...
inboxQueue
	...
	.on('error', err => inboxLogger.error(`error ${err}`, { e: renderError(err) }))
...
dbQueue
	...
	.on('error', err => dbLogger.error(`error ${err}`, { e: renderError(err) }))
