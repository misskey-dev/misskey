...
<div class="urbixznjwwuukfsckrwzwsqzsxornqij">
	...
	<div>
		...
		<div class="card map">
			...
			<div>
				<div class="random" v-if="game.settings.map == null"><font-awesome-icon icon="dice"/></div>
				<div class="board" v-else :style="{ 'grid-template-rows': `repeat(${ game.settings.map.length }, 1fr)`, 'grid-template-columns': `repeat(${ game.settings.map[0].length }, 1fr)` }">
					<div v-for="(x, i) in game.settings.map.join('')"
						...
						<template v-if="x == 'b'"><template v-if="$store.state.device.darkmode"><font-awesome-icon icon="circle R"/></template><template v-else><font-awesome-icon icon="circle"/></template></template>
						<template v-if="x == 'w'"><template v-if="$store.state.device.darkmode"><font-awesome-icon icon="circle"/></template><template v-else><font-awesome-icon icon="circle R"/></template></template>
					</div>
				</div>
			</div>
		</div>
