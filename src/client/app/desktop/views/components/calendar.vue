...
<div class="mk-calendar" :data-melt="design == 4 || design == 5">
	<template v-if="design == 0 || design == 1">
		<button @click="prev" title="%i18n:@prev%"><font-awesome-icon icon="chevron-circle-left"/></button>
		<p class="title">{{ '%i18n:@title%'.replace('{1}', year).replace('{2}', month) }}</p>
		<button @click="next" title="%i18n:@next%"><font-awesome-icon icon="chevron-circle-right"/></button>
	</template>
