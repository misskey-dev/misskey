<template>
<mk-ui>
	<span slot="header"><span style="margin-right:4px;"><font-awesome-icon icon="home"/></span>%i18n:@dashboard%</span>
	<template slot="func">
		<button @click="customizing = !customizing"><font-awesome-icon icon="cog"/></button>
	</template>
	<main>
		<template v-if="customizing">
			...
			>
				<div v-for="widget in widgets" class="customize-container" :key="widget.id">
					<header>
						<span class="handle"><font-awesome-icon icon="bars"/></span>{{ widget.name }}<button class="remove" @click="removeWidget(widget)"><font-awesome-icon icon="times"/></button>
					</header>
