...
- `note_favorite` テーブルおよび `user_note_pining` テーブルにインデックスを追加しました。
  大規模なサーバーでは、マイグレーション中にこれらのテーブルへのアクセスがブロックされる場合があります。
  ブロックを避けたい場合は、マイグレーション実行前に環境変数 `MISSKEY_MIGRATION_CREATE_INDEX_CONCURRENTLY=1` を設定してください。
  この設定により `CREATE INDEX CONCURRENTLY` が使用され、テーブルロックなしでインデックスが作成されます（ただし処理時間は長くなります）。
