...
### Note
- リアクションデッキが「リアクション用」と「投稿用」に分かれました。以前のリアクションデッキは「リアクション用」として使用されます。  

	**影響：**  
	それにより、投稿フォームから表示される絵文字ピッカーのデッキがリセットされたように感じるかもしれません（新設された投稿用のデッキが使われるため）。   
	投稿用のデッキをアップデート前の状態にするには、以下の手順で操作します。

	1. 「設定」メニューに移動し、「リアクション・絵文字」タブを選択します。
	2. 「絵文字デッキ」のタブを選択します。
	3. 「リアクションデッキからコピーする」ボタンを押すことで、アップデート前の状態に戻すことができます。
...
- Enhance: リアクション用デッキと投稿時の絵文字入力用デッキを分けて持てるように #12560
