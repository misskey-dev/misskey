...
### ✨Improvements
* パスワード無しログイン機能
* GroundpolisPagesにイベント送信ボタンを追加
* GroundpolisPagesでボタンを目立たせることができるように
* ページをピン留めできるように
* デッキのカラム内でページを見れるように
...
* ServiceWorker有効化
* GroundpolisPagesでリストの要素数を取得する関数を追加
...
* インスタンス名を空にしてもGroundpolisロゴが表示されない問題を修正
...
* GroundpolisPagesでNAME環境変数がNULLにならないように
* GroundpolisPagesにNULL環境変数を追加
* GroundpolisPagesで変数を並べ替えられるように
* GroundpolisPagesのテキストのリスト内で変数埋め込みできるように
...
--------------------
### 🐛Fixes
* GroundpolisPagesのリストから選択関数が使えない問題を修正

11.11.0 (2019/05/05)
--------------------
### ✨ Improvements
* GroundpolisPagesにリストから選択関数を追加
* GroundpolisPagesに確率を指定できるテキストランダム選択関数を追加
* 外部サービス連携ログインリンクにアイコン追加

### 🐛Fixes
* GroundpolisPagesでifを入れ子にできなくなっていた問題を修正
* GroundpolisPagesで数値入力を作成するとテキスト入力になる問題を修正
* 外部サービス連携に関する問題を修正

11.10.1 (2019/05/04)
--------------------
### 🐛Fixes
* GroundpolisPagesでページブロックを削除できなくなっていた問題を修正
...
### ✨ Improvements
* GroundpolisPagesに割った余りを求める関数を追加
...
-------------------
### ✨ Improvements
* GroundpolisPagesで編集時にページブロックをドラッグで並べ替えられるように
* GroundpolisPagesにカウンターボタンブロックを追加
...
-------------------
### ✨ Improvements
* GroundpolisPagesで関数を作成できるように
* GroundpolisPagesでソースを表示できるように
* GroundpolisPagesにシードを与えるランダム関数を追加
* GroundpolisPagesに複数行テキストをテキストのリストに変換する関数を追加
...
-------------------
### ✨ Improvements
* GroundpolisPagesに ifブロック を追加
* GroundpolisPagesに テキストエリア を追加
* GroundpolisPagesに 複数行テキスト入力 を追加
* GroundpolisPagesに 投稿フォーム を追加
* GroundpolisPagesに 変換系関数 を追加
* GroundpolisPagesに 環境変数 URL を追加
* GroundpolisPagesでボタンやスイッチなどのテキストに変数使えるように
...
### 🐛Fixes
* GroundpolisPagesで数値入力が文字列として扱われる問題を修正
* デザインの調整

11.5.1 (2019/04/29)
-------------------
### 🐛Fixes
* GroundpolisPagesで環境変数を別の変数内で使えない問題を修正
* GroundpolisPagesで値が0の変数が表示されない問題を修正
...
-------------------
### 🐛Fixes
* Metaタグの application-name を Groundpolis で固定するように修正
...
* MFMの解析を強化
* Groundpolis以外のインスタンスからGroundpolisの投稿を見たときに改行が多い問題を修正
* Groundpolis以外のインスタンスからGroundpolisの投稿を見たときにメンションのURLが展開されるのを修正
...
* トークのメッセージを削除できるように
* リアクションを取り消せるように
* Groundpolis以外のソフトウェアからの「Like」アクティビティをプリンではなく「いいね」として扱うように
* i18nの修正
* バグ修正
* など
...
* リモートGroundpolisユーザーの情報が登録/更新出来なくなっていたのを修正
...
Groundpolis v8.64.0 を使っている方は、9.0.0に際しては特にすべきことはありません。
Groundpolis v8.64.0 に満たないバージョンをお使いの方は、一旦8.64.0にアップデートして(そして起動して)から9.0.0に再度アップデートしてください。
