...
- Fix: ドライブのソートが「登録日（昇順）」の場合に正しく動作しない問題を修正  
  - `drive/files` APIのページネーションでは limit/offset を使用するようになりました。sinceId/untilId/sinceDate/untilDate の実装は現時点では残置していますが、上記のような不具合があるため非推奨（deprecated）としました。将来的に削除される可能性があります。
