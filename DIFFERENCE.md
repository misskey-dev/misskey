...
### Fix
- **プライバシー設定画面の不具合を修正**
  - 「オンライン状態を隠す」設定項目が重複していた問題を修正
  - 「アクティブ状態の公開範囲」が変更できなかった問題を修正
  - 「アクティブ状態の公開範囲」と「オンライン状態を隠す」の表示順序を修正
  - 「ノートの公開範囲」に誤った説明文が表示されていた問題を修正
  - `v-model`のcomputed setterと`@update:modelValue`の二重呼び出しを修正
- **新規ユーザーのデフォルトプライバシー設定を強化**
  - 「アクティブ状態の公開範囲」のデフォルト値を「相互フォロー」から「非公開」に変更
  - 既存ユーザーの設定には影響なし
- **mCaptchaが動作しない問題を修正**
  - `@mcaptcha/vanilla-glue`を`0.1.0-rc2`から`0.1.0-alpha-3`にダウングレード
  - 2025.12.2で依存関係が`rc2`にアップグレードされた際、APIの変更により動作しなくなっていた
  - `alpha-3`は実績があり安定版としてnpmの`latest`タグが付いている
  - 将来的にmCaptchaが1.0.0正式版をリリースした際は、その仕様に合わせて再度対応予定

## 2025.12.2-yami-1.9.29

### Misskey 2025.12.2への追従

本家Misskey 2025.12.2をdevelopブランチにマージしました。詳細はCHANGELOG.mdを参照してください。
...
- 機能「ユーザーのサーバー情報をアイコンのみにできるように」削除
...
本家Misskey 2025.12.0をdevelopブランチにマージしました。詳細はCHANGELOG.mdを参照してください。

#### yamisskey独自機能への影響

今回のマージではコンフリクトはほぼなく、yamisskey独自機能（やみノート、時限消滅ノート、予約投稿等）への影響はありません。

## 2025.11.1-yami-1.9.29
...
- **投稿フォームのリセット機能**: やみノート設定・削除予定時刻が正しくリセットされるか確認
- **チャンネルストリーミング**: ロックダウン設定とyamisskey未ログイン制限機能の整合性確認
- **AiScript関連**: `null`引数のハンドリング修正がやみノート関連プラグインに影響しないか確認

#### AI レビュー指示

@claude このPRをレビューしてください。特に以下を確認：
1. 投稿フォームリセット時のやみノート設定・削除予定時刻のリセット動作
2. チャンネル内でやみノートボタンが非表示になり強制的に非やみノートになっているか
3. ロックダウン設定と未ログイン制限機能の競合がないか
4. やみタイムライン・時限消滅ノート・予約投稿機能の正常動作
